# Pakman — Design v0.1 (Architecture Sketch)
...
Pakman is not a PPM clone. PPM is a strong product; the honest goal is to fill the slots PPM doesn't serve well, not to out-Posit Posit.

**What PPM does well (pakman does not try to match in v1)**
...
**v1 in one sentence:** a small open-source server that hosts your organisation's **internal** R packages with a CRAN-protocol-compatible endpoint. Users point their R at both pakman (for internal pkgs) and CRAN / PPM / r-universe (for public pkgs), as two entries in `repos`.

**What pakman v1 ships as concrete differentiators**

| Gap in PPM | Pakman v1's angle |
|---|---|
...
| Git-source workflow is bolt-on | Gitea / Git host + CI is the primary publishing path by design — CI pushes built tarballs to pakman. |
...
|---|---|
| **Air-gap deploy + bundled CRAN / Bioconductor mirror** | The defining v1.x feature. Sync-bundle format, snapshot-update cadence, signing, and "updating CRAN on prod" workflow are a whole design project. v1 hosts internal packages only; users point R at upstream CRAN / PPM alongside pakman. See §10. |
...
| Item | Pakman's role instead |
|---|---|
| Rust client CLI (`add` / `sync` / `lock` / `tree` / `run`) | **uvr** and **rv** already occupy this space. Pakman ships a well-documented HTTP API + OpenAPI spec (§7) so those tools can support pakman as a publish destination. Building a third R client would be a different project. |
...
**What pakman explicitly does NOT try to match ever**
...
- Becoming "the uv of R." Pakman is a server; the client story is uvr / rv / existing R tools.

**One-line positioning (v1)**

> An open-source, single-binary R package registry for **internal** R packages — think "private CRAN for your organisation". Users point their R at both pakman (for internal) and upstream CRAN / PPM / r-universe (for public). **Air-gap deploy with a bundled CRAN mirror is the committed v1.x roadmap**.
...
Pakman is a focused OSS project serving a real but narrow niche. It is not trying to be uv or to replace PPM. It aims to be the obvious choice for the first-tier targets above, and to have a clear on-ramp to the second tier via the v1.x air-gap roadmap.
...
| Area | Decision |
|---|---|
| Client side | **Out of scope for v1.** Existing R tools (base, renv, pak, uvr, rv) drive pakman via CRAN-protocol compat. A Rust CLI is roadmap, not v1. |
...
| CRAN / Bioc handling | **Not hosted by pakman in v1.** Users configure R with pakman + upstream CRAN/PPM/Bioc as separate `repos`. Mirroring + air-gap is on the roadmap. |
...
1. **`pakman-server`** — single Go binary with embedded static assets (HTML templates, CSS, OpenAPI spec). Linux amd64 at launch; arm64 close behind.
...
5. **Migration tooling** — `pakman admin import --from drat <url>` to ingest an existing drat repo into a channel; `pakman admin import --from git <url> --branch <b>` to ingest a gitea/GitHub R-package source (runs `R CMD build` locally and publishes the source tarball). Adoption is gated on a clean migration from whatever users run today.
6. **5-minute quickstart documentation** — from `docker run pakman-server` to publishing the first package via curl. README + `/docs/quickstart.md`. Explicit success criterion: a new user runs five commands and sees their package in the channel list.
...
```
    ...
    └───────────────────────┬───────────────────────┘
          ...
          │  configured with BOTH pakman AND        │
...
**Channels do not overlay each other on the server.** Clients decide composition — they configure an ordered list of channel URLs in `repos =` (pakman alongside upstream CRAN / PPM), exactly like standard R against multiple CRAN mirrors. This keeps the server dumb and moves "which sources this project uses" into the project config, where it belongs.
...
**Trust model (v1):** the server trusts CI to have run the build honestly. Auth tokens scope *who can publish where*. Signed attestations (Sigstore) are roadmap. For a single-tenant on-prem deploy where CI is on the same LAN as pakman, this is acceptable. Operators who need stronger guarantees can gate publish behind an approval step in CI — that's a CI concern, not a pakman concern.

---

## 6. Clients (v1)

**Guiding principle: the HTTP API is the publishing UX.** No bespoke pakman CLI, no R-side helper package, no IDE extension in v1. Anyone publishing uses `curl` (or any HTTP client); anyone consuming uses existing R tooling against the CRAN-compat endpoints. Wrappers in R, Rust, VS Code, etc. are things the community or a later version can build on top of a stable, documented API.
...
```r
# Typical v1 setup: pakman for internal pkgs, upstream CRAN/PPM for public pkgs
options(repos = c(
  PAKMAN = "https://pakman.corp/prod/",
  CRAN   = "https://packagemanager.posit.co/cran/latest"   # or any CRAN mirror / upstream Bioc
))
install.packages(c("ggplot2", "myInternal"))   # dplyr → CRAN; myInternal → pakman
```
...
```r
# pak
pak::repo_add(PAKMAN = "https://pakman.corp/prod/")
...
```sh
curl -X POST https://pakman.corp/api/v1/publish \
  -H "Authorization: Bearer $PAKMAN_TOKEN" \
  -F "channel=dev" \
...
A helper gitea-actions / GitHub-actions workflow shipping with pakman will wrap this. A first-party `pakman` CLI (Rust, uv-shaped) is roadmap once the server stabilises — see §12.
...
v1 does **not** run builds on the pakman host. CI builds artifacts per cell and uploads them. The matrix YAML defines which cells pakman *accepts* uploads for — the same identity flows into CAS keys, lockfile platform markers, and download URLs whether the artifact was built by server-side Docker (future) or by CI (v1).
...
- the `pakman.lock` platform marker,
...
Static YAML at `/etc/pakman/matrix.yaml` (path configurable). Loaded at startup; changes require a server restart. Source of truth for the cells the server will index and serve.
...
v1 ships a reference workflow template (not a maintained action) at `/examples/ci/publish.yml` in the pakman repository. Users copy it into `.github/workflows/` (or `.gitea/workflows/`) and adapt the matrix + channel mapping. The publish step is plain `curl` against `/api/v1/publish` — no maintained pakman action artifact, no additional CLI dependency.
...
3. **`publish`** — plain Ubuntu runner (no R), downloads all artifacts, builds the manifest JSON with `jq`, curls multipart to pakman. Uses `--fail-with-body` so a 4xx from the server surfaces the `error_code` / `message` / `hint` from §7.2.

**Reference YAML:**

```yaml
# /examples/ci/publish.yml — reference template for publishing an R package to pakman.
# Works on GitHub Actions and Gitea Actions. Requires:
#   - Repo secret PAKMAN_TOKEN with publish:<channel> scope.
#   - Repo variable PAKMAN_SERVER (e.g. https://pakman.corp).
#   - Cells below must match the server's matrix.yaml (GET /api/v1/cells).

name: Publish to pakman
  ...
  publish:
    ...
    steps:
      ...
      - uses: actions/download-artifact@v4
        with: { path: artifacts }
      - env:
          PAKMAN_SERVER: ${{ vars.PAKMAN_SERVER }}
          PAKMAN_TOKEN:  ${{ secrets.PAKMAN_TOKEN }}
        ...
        run: |
          ...
          curl --fail-with-body -X POST \
            ...
            "${CURL_FILES[@]}" \
            "$PAKMAN_SERVER/api/v1/publish"
```

**What users customise:**

- The `matrix.cell` list — must match pakman's `matrix.yaml`.
...
- Secrets/variables — `PAKMAN_TOKEN` and `PAKMAN_SERVER` live in the repo's CI settings.
...
- A maintained `pakman-project/publish@v1` action. Wrapping the curl step into an action is left as a later polish; the template stays curl-based to keep the HTTP API as the canonical publish surface (§6).
...
- `pakman admin cells list` — configured cells and their stats (count of pkgs with binaries for this cell, coverage %).
- `pakman admin cells show <name>` — details including packages with missing binaries.
- `pakman admin reindex` — rebuild `PACKAGES` indices from CAS (recovery op).
...
- **Per-package cell opt-out** (e.g., a `pakman:` block in DESCRIPTION saying "skip `ubuntu-20.04` cells"). Useful, but not before the basic flow is solid.
...
Channels are the unit of publication and pakman's primary namespace. The `Channel` entity is defined in §3; publish / yank / delete endpoints live in §7. This section covers channel configuration, mutability policy, defaulting, and deletion semantics.

### 9.1 Channel config

Static YAML at `/etc/pakman/channels.yaml` (path configurable). Loaded at startup; changes require restart. Shipped default:
...
- **`mutable`** — a re-publish to `(name, version)` replaces the prior bytes. Old source and binary blobs in the CAS become orphaned; `pakman admin gc` removes them. Only the latest bytes are retained. The `/events` feed (§7.4) is the audit trail of what was published when; prior bytes are not preserved. Recommended for `dev`, `test`.
...
The channel flagged `default: true` is served as a URL alias at the root so clients can point at pakman without specifying a channel:

```
https://pakman.corp/4.4/src/contrib/PACKAGES        # aliased → /prod/4.4/...
https://pakman.corp/prod/4.4/src/contrib/PACKAGES   # explicit — identical bytes
```
...
options(repos = c(
  PAKMAN = "https://pakman.corp/",
  CRAN   = "https://packagemanager.posit.co/cran/latest"
))

# Package authors iterating in dev:
options(repos = c(
  PAKMAN_DEV = "https://pakman.corp/dev/",
  PAKMAN     = "https://pakman.corp/",
  CRAN       = "https://packagemanager.posit.co/cran/latest"
))
```
...
- Removes the channel index entry entirely. CAS blobs become orphaned; `pakman admin gc` reclaims them.
...
This is a client contract, not a server one. Pakman just serves whatever bytes are currently bound to the channel entry.
...
- `pakman admin channels list` — configured channels with their policies and stats.
- `pakman admin gc` — garbage-collect orphaned CAS blobs. On-demand, not scheduled.
...
- **Scheduled GC.** Operator runs `pakman admin gc` when disk pressure matters.

---

## 10. CRAN mirror + air-gap deploy (roadmap, not v1)

Not in v1. Users configure R to hit pakman for internal packages and upstream CRAN / PPM / r-universe for public ones — see §6. Captured here as the design inputs so the future feature lands without retrofitting.
...
- A CRAN mirror is just another pakman channel (per §4), served read-only from the publish API's perspective.
...
- Sync-bundle format: a tarball containing the delta since the previous snapshot, plus metadata (signed manifest, source hashes, content-addressable layout). Importable on a disconnected host via `pakman-server import-bundle`.
...
- **First-party Rust client CLI** (`add` / `sync` / `lock` / `tree` / `run`). [uvr](https://github.com/nbafrank/uvr) and [rv](https://github.com/A2-ai/rv) already occupy this space. Pakman's contribution is to be a first-class, well-documented publish destination via the HTTP API + OpenAPI spec. Building a third R client would be a different project.
- **Content-addressable client-side store with hardlinks.** Client-side concern; whichever client the user picks inherits it (pnpm / uv-style is the reference). Not pakman's responsibility.
- **PubGrub / pak-style resolver.** Same — client-side.
- **R task runner** (`pakman run`-style). Out of scope; users have `targets`, `make`, or the R client of their choice.
- Any promotion workflow / approval gates / stage-aware resolver — CI owns this, not pakman.
...
- Trying to be "the uv of R." Pakman is a server; the client story lives with uvr / rv / existing R tools.
